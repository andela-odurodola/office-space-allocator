...
class Fellow(Person):
    ...
    def get_fellow_identifier(self):
        fellow_id = 'F'.join([random.choice(string.ascii_letters + string.digits) for n in range(1,5)])
...
class Staff(Person):
    ...
    def get_staff_identifier(self):
        staff_id = 'S'.join([random.choice(string.ascii_letters + string.digits) for n in range(1, 5)])
